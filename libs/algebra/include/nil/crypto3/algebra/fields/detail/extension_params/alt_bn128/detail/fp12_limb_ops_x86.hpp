...
namespace nil::crypto3::algebra::fields::detail::alt_bn128_fp12_limb_ops {
    inline void subtract_limbs_x86(limb_array &result, const limb_array &other) {
        asm volatile(
            ...
            "subq %%rax, " PTR(result, 0) "\n"
            "movq " PTR(other, 1) ", %%rax\n"
            "sbbq %%rax, " PTR(result, 1) "\n"
            "movq " PTR(other, 2) ", %%rax\n"
            "sbbq %%rax, " PTR(result, 2) "\n"
            "movq " PTR(other, 3) ", %%rax\n"
            "sbbq %%rax, " PTR(result, 3) "\n"
            "movq " PTR(other, 4) ", %%rax\n"
            "sbbq %%rax, " PTR(result, 4) "\n"
            "movq " PTR(other, 5) ", %%rax\n"
            "sbbq %%rax, " PTR(result, 5) "\n"
            "movq " PTR(other, 6) ", %%rax\n"
            "sbbq %%rax, " PTR(result, 6) "\n"
            "movq " PTR(other, 7) ", %%rax\n"
            "sbbq %%rax, " PTR(result, 7) "\n"
            "movq " PTR(other, 8) ", %%rax\n"
            "sbbq %%rax, " PTR(result, 8) "\n"
            :
            : [result]"r"(result.data()),
              [other]"r"(other.data())
            : "rax", "cc", "memory"
