...
#include <nil/crypto3/algebra/fields/detail/extension_params/bls12/detail/fp12_limb_ops.hpp>

namespace nil::crypto3::algebra::fields::detail {

    template<typename BaseField, typename ExtensionParams>
    struct bls12_fp12_fast_multiply {
        ...
        using limb_array = bls12_fp12_limb_ops::limb_array;
        using limb = bls12_fp12_limb_ops::limb;
        ...
        struct fp2_base {
            ...
            static void add_mod(fp2_base &z, const fp2_base &x, const fp2_base &y) {
                bls12_fp12_limb_ops::fp2_base_add_mod<base_field_type>(z.data.data(), x.data.data(), y.data.data());
            }
        ...
        struct fp2_dbl {
            ...
            static void add_mod(fp2_dbl &z, const fp2_dbl &x, const fp2_dbl &y) {
                bls12_fp12_limb_ops::add_8_limbs_mod<base_field_type>(z.data[0], x.data[0], y.data[0]);
                bls12_fp12_limb_ops::add_8_limbs_mod<base_field_type>(z.data[1], x.data[1], y.data[1]);
            }

            static void sub_mod(fp2_dbl &z, const fp2_dbl &x, const fp2_dbl &y) {
                bls12_fp12_limb_ops::subtract_8_limbs_mod<base_field_type>(z.data[0], x.data[0], y.data[0]);
                bls12_fp12_limb_ops::subtract_8_limbs_mod<base_field_type>(z.data[1], x.data[1], y.data[1]);
            }
            ...
            void sub_pre(const fp2_dbl &other) {
                bls12_fp12_limb_ops::fp2_sub_pre<base_field_type>(data.data(), other.data.data());
            }

            static void add_mul_pre(fp2_dbl &result, const fp2_base &a, const fp2_base &b, const fp2_base &c,
                                    const fp2_base &d) {
                bls12_fp12_limb_ops::fp2_add_mul_pre<base_field_type>(result.data.data(), a.data.data(),
                                                                          b.data.data(), c.data.data(), d.data.data());
            }

            static void mul_pre(fp2_dbl &result, const fp2_base &x, const fp2_base &y) {
                bls12_fp12_limb_ops::fp2_mul_pre<base_field_type>(result.data.data(), x.data.data(), y.data.data());
            }

            // dst = src * xi + addend
            static void mul_xi_add(fp2_dbl &dst, const fp2_dbl &src, const fp2_dbl &addend) {
                bls12_fp12_limb_ops::fp2_mul_xi_add<base_field_type>(dst.data.data(), src.data.data(),
                                                                         addend.data.data());
            }

            // src = src * xi + addend
            static void mul_xi_add_modify_src(fp2_dbl &src, const fp2_dbl &addend) {
                bls12_fp12_limb_ops::fp2_mul_xi_add_modify_src<base_field_type>(src.data.data(),
                                                                                    addend.data.data());
            }

            // addend = src * xi + addend
            static void mul_xi_add_modify_addend(fp2_dbl &addend, const fp2_dbl &src) {
                bls12_fp12_limb_ops::fp2_mul_xi_add_modify_addend<base_field_type>(addend.data.data(),
                                                                                       src.data.data());
            }

            void to_non_residue(non_residue_type &ret) const {
                bls12_fp12_limb_ops::montgomery_reduce<base_field_type>(
        ...
        struct fp6_dbl {
            ...
            fp6_dbl operator+(const fp6_dbl &other) const {
                return fp6_dbl(data[0] + other.data[0], data[1] + other.data[1], data[2] + other.data[2]);
            }
            ...
            static void add_mul_pre(fp6_dbl &result, const fp6_base &a, const fp6_base &b, const fp6_base &c,
                                    const fp6_base &d) {
                fp6_base x, y;
