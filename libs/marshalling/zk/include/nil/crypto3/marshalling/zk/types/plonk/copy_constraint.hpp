    ...
    namespace crypto3 {
        namespace marshalling {
            namespace types {
                ...
                using plonk_copy_constraint = nil::marshalling::types::bundle<
                    TTypeBase,
                    std::tuple<typename variable<
                                   TTypeBase,
                                   nil::crypto3::zk::snark::plonk_variable<typename FieldType::value_type>>::type,
                               typename variable<
                                   TTypeBase,
                                   nil::crypto3::zk::snark::plonk_variable<typename FieldType::value_type>>::type>>;
                ...
                plonk_copy_constraint<typename nil::marshalling::field_type<Endianness>, FieldType>
                    fill_plonk_copy_constraint(
                        const nil::crypto3::zk::snark::plonk_copy_constraint<FieldType> &copy_constraint) {
                    ...
                    return result_type(
                        std::make_tuple(fill_variable<Endianness, VariableType>(copy_constraint.first),
                                        fill_variable<Endianness, VariableType>(copy_constraint.second)));
                }

                template<typename Endianness, typename FieldType>
                nil::crypto3::zk::snark::plonk_copy_constraint<FieldType> make_plonk_copy_constraint(
                    const plonk_copy_constraint<typename nil::marshalling::field_type<Endianness>, FieldType>
                ...
                template<typename TTypeBase, typename FieldType>
                using plonk_copy_constraints =
                    nil::marshalling::types::standard_array_list<TTypeBase,
                                                                 plonk_copy_constraint<TTypeBase, FieldType>>;

                template<typename Endianness, typename FieldType>
                plonk_copy_constraints<nil::marshalling::field_type<Endianness>, FieldType> fill_plonk_copy_constraints(
                    const std::vector<nil::crypto3::zk::snark::plonk_copy_constraint<FieldType>> &constraints) {
                    using TTypeBase = nil::marshalling::field_type<Endianness>;
                ...
                std::vector<nil::crypto3::zk::snark::plonk_copy_constraint<FieldType>>
                    make_plonk_copy_constraints(const plonk_copy_constraints<nil::marshalling::field_type<Endianness>,
                                                                             FieldType> &filled_constraints) {
                    ...
                    for (std::size_t i = 0; i < filled_constraints.value().size(); i++) {
                        constraints.emplace_back(
                            make_plonk_copy_constraint<Endianness, FieldType>(filled_constraints.value()[i]));
                    }
                    return constraints;
                }
            }    // namespace types
