...
/*`[h5 Using Boost.Multiprecision `cpp_float` types for numerical calculations with higher precision than __fundamental
`long double`.]
...
[@https://www.boost.org/doc/libs/release/libs/math/doc/html/constants.html Boost.Math collection of high-precision
constants], we need some includes:
*/
...
int main() {
    ...
    */
    ...
    /*`By default, output would only show the standard 6 decimal digits,
     so set precision to show all 50 significant digits, including any trailing zeros (to show the full implied 50 digit
     precision).
    */
    ...
    */
    ...
    but beware of less-than-expected precision from construction or conversion from `double` or other lower precision
    types. This is a mistake that is very easy to make, and very difficult to detect because the difference in precision
    is only visible after about the 17th decimal digit.

    We can show this by constructing a fraction one seventh from `double`:
    */
    cpp_bin_float_50 seventh_0 =
        cpp_bin_float_50(1 / 7);    // Avoid the schoolboy-error `double d7 = 1 / 7;` giving zero!
    ...
    The 'random' digits after digit 17 are from the `cpp_bin_float_50` representation of the `double` value
    0.14285714285714 which is different from the 'better' `cpp_bin_float_50` representation of the fraction 1/7
    */
 ...
 */
    ...
    std::cout << "cpp_bin_float_50 f1(cpp_bin_float_50(1234) / 1000) = " << f1
              << std::endl;                // cpp_bin_float_50 f1(cpp_bin_float_50(1234) / 1000)
                                           // = 1.2340000000000000000000000000000000000000000000000
                                           /*`or
                                            */
    const cpp_bin_float_50 f2("1.234");    // Construct from decimal digit string.
    std::cout << "cpp_bin_float_50 f2(\"1.234\") = " << f2
              << std::endl;    // cpp_bin_float_50 f2("1.234") = 1.2340000000000000000000000000000000000000000000000

    /*`that are different from constructing from a `double` with value 1.234
     */
    const cpp_bin_float_50 f3(cpp_bin_float_50(1.234));
    std::cout << "cpp_bin_float_50 f3(cpp_bin_float_50(1.234)) = " << f3
              << std::endl;    // 1.2339999999999999857891452847979962825775146484375
