...
namespace my_math {
    template<class T>
    T chebyshev_t(const std::int32_t n, const T& x);

    template<class T>
    T chebyshev_t(const std::uint32_t n, const T& x, std::vector<T>* vp);

    template<class T>
    bool isneg(const T& x) {
        return (x < T(0));
    }

    template<class T>
    const T& zero() {
        static const T value_zero(0);
        return value_zero;
    }
    template<class T>
    const T& one() {
        static const T value_one(1);
        return value_one;
    }
    template<class T>
    const T& two() {
        static const T value_two(2);
        return value_two;
    }
}    // namespace my_math
...
namespace orthogonal_polynomial_series {
    template<typename T>
    static inline T orthogonal_polynomial_template(const T& x,
                                                   const std::uint32_t n,
                                                   std::vector<T>* const vp = static_cast<std::vector<T>*>(0u)) {
...
template<class T>
T my_math::chebyshev_t(const std::int32_t n, const T& x) {
    if (my_math::isneg(x)) {
...
T my_math::chebyshev_t(const std::uint32_t n, const T& x, std::vector<T>* const vp) {
    return orthogonal_polynomial_series::orthogonal_polynomial_template(x, static_cast<std::int32_t>(n), vp);
}
...
namespace examples {
    namespace nr_006 {
        template<typename T>
        class hypergeometric_pfq_base : private boost::noncopyable {
        public:
            ...
            virtual T series() const {
                ...
                chebyshev_t(static_cast<std::uint32_t>(C.size()), z_shifted, &chebyshev_t_shifted_values);
                ...
                const T scale = std::accumulate(C.begin(), C.end(), T(0), [&b_neg](T scale_sum, const T& ck) -> T {
                ...
                const T sum = std::inner_product(C.begin(), C.end(), chebyshev_t_shifted_values.begin(), T(0));
        ...
        protected:
            ...
            hypergeometric_pfq_base(const T& z, const T& w) : Z(z), W(w), C(0u) {
            }

            virtual std::int32_t N() const {
                return static_cast<std::int32_t>(util::digit_scale<T>() * 500.0F);
            }
        };

        template<typename T>
        class ccoef4_hypergeometric_0f1 : public hypergeometric_pfq_base<T> {
        public:
            ccoef4_hypergeometric_0f1(const T& c, const T& z, const T& w) : hypergeometric_pfq_base<T>(z, w), CP(c) {
            }
            ...
            virtual void ccoef() const {
                ...
                for (std::int32_t K = static_cast<std::int32_t>(0); K < N1; ++K) {
                    ...
                    const T term = (A2 - ((A3 * DIVFAC) * X1)) + ((A2 * X1) * ((1 + (C1 + X1)) * Z1)) +
        ...
        template<typename T>
        class ccoef1_hypergeometric_1f0 : public hypergeometric_pfq_base<T> {
        public:
            ccoef1_hypergeometric_1f0(const T& a, const T& z, const T& w) : hypergeometric_pfq_base<T>(z, w), AP(a) {
            }
        ...
        private:
            ...
            virtual std::int32_t N() const {
                return static_cast<std::int32_t>(util::digit_scale<T>() * 1600.0F);
            }
        };

        template<typename T>
        class ccoef3_hypergeometric_1f1 : public hypergeometric_pfq_base<T> {
        public:
            ccoef3_hypergeometric_1f1(const T& a, const T& c, const T& z, const T& w) :
                hypergeometric_pfq_base<T>(z, w), AP(a), CP(c) {
            }
        ...
        template<typename T>
        class ccoef6_hypergeometric_1f2 : public hypergeometric_pfq_base<T> {
        public:
            ccoef6_hypergeometric_1f2(const T& a, const T& b, const T& c, const T& z, const T& w) :
                hypergeometric_pfq_base<T>(z, w), AP(a), BP(b), CP(c) {
            }
            ...
            virtual void ccoef() const {
                ...
                for (std::int32_t K = static_cast<std::int32_t>(0); K < N1; ++K) {
                    ...
                    const T PART2 = (A2 * (X + 2)) * ((((TWO_X + 1) * PP) / X_PLUS_1) - ((QQ * 4) * (PP + 1)) +
                                                      (((TWO_X + 3) * (PP + 2)) / X_PLUS_3) +
                                                      ((Z1 * 2) * (SS - (QQ * (X_PLUS_1 + BP)) * (X_PLUS_1 + CP))));
                    const T PART3 = A3 * ((((X_PLUS_3 - AP) - (QQ * (X_PLUS_4 - AP))) * 2) +
                                          (((QQ * Z1) * (X_PLUS_4 - BP)) * (X_PLUS_4 - CP)));
        ...
        template<typename T>
        class ccoef2_hypergeometric_2f1 : public hypergeometric_pfq_base<T> {
        public:
            ccoef2_hypergeometric_2f1(const T& a, const T& b, const T& c, const T& z, const T& w) :
                hypergeometric_pfq_base<T>(z, w), AP(a), BP(b), CP(c) {
            }
            ...
            virtual void ccoef() const {
                ...
                for (std::int32_t K = static_cast<std::int32_t>(0); K < N1; ++K) {
                    ...
                    // The terms have been slightly re-arranged resulting in lower complexity.
                    // Parentheses have been added to avoid reliance on operator precedence.
                    const T PART1 = (A1 * X1) * (2 - (((AP + X1) * (BP + X1)) * ((T(X3) / X_PLUS_2) * XAB)) +
                                                 ((CP + X) * (XAB * Z1)));
                    const T PART2 =
                        (A2 * XAB) * ((X3A * X3B) - (X3 * ((X3A + X3B) - 1)) + (((3 - CP) + X) * (X1 * Z1)));
        ...
        template<class T>
        T luke_ccoef4_hypergeometric_0f1(const T& a, const T& x);
        template<class T>
        T luke_ccoef1_hypergeometric_1f0(const T& a, const T& x);
        template<class T>
        T luke_ccoef3_hypergeometric_1f1(const T& a, const T& b, const T& x);
        template<class T>
        T luke_ccoef6_hypergeometric_1f2(const T& a, const T& b, const T& c, const T& x);
        template<class T>
        T luke_ccoef2_hypergeometric_2f1(const T& a, const T& b, const T& c, const T& x);
    }    // namespace nr_006
}    // namespace examples
...
int main() {
    ...
    std::for_each(hypergeometric_0f1_results.begin(), hypergeometric_0f1_results.end(), [&i, &a](mp_type& new_value) {
    ...
    std::for_each(hypergeometric_0f1_results.begin(), hypergeometric_0f1_results.end(), [](const mp_type& h) {
    ...
    // Compare with the Mathematica command:
    // Table[N[HypergeometricPFQ[{3/7}, {}, -(i*EulerGamma)], 100], {i, 1, 20, 1}]
    std::for_each(hypergeometric_1f0_results.begin(), hypergeometric_1f0_results.end(), [&i, &a](mp_type& new_value) {
        const mp_type x(-(boost::math::constants::euler<mp_type>() * i));
    ...
    std::for_each(hypergeometric_1f0_results.begin(), hypergeometric_1f0_results.end(), [](const mp_type& h) {
        std::cout << std::setprecision(DIGIT_COUNT) << h << std::endl;
    });
    ...
    std::for_each(
        hypergeometric_1f1_results.begin(), hypergeometric_1f1_results.end(), [&i, &a, &b](mp_type& new_value) {
            const mp_type x(-(boost::math::constants::euler<mp_type>() * i));
    ...
    std::for_each(hypergeometric_1f1_results.begin(), hypergeometric_1f1_results.end(), [](const mp_type& h) {
        std::cout << std::setprecision(DIGIT_COUNT) << h << std::endl;
    });
    ...
    std::for_each(
        hypergeometric_1f2_results.begin(), hypergeometric_1f2_results.end(), [&i, &a, &b, &c](mp_type& new_value) {
            const mp_type x(-(boost::math::constants::euler<mp_type>() * i));
    ...
    std::for_each(hypergeometric_1f2_results.begin(), hypergeometric_1f2_results.end(), [](const mp_type& h) {
        std::cout << std::setprecision(DIGIT_COUNT) << h << std::endl;
    });
    ...
    // Table[N[HypergeometricPFQ[{3/7, 2/3}, {1/4}, -(i * EulerGamma)], 100], {i, 1, 20, 1}]
    std::for_each(
        hypergeometric_2f1_results.begin(), hypergeometric_2f1_results.end(), [&i, &a, &b, &c](mp_type& new_value) {
            const mp_type x(-(boost::math::constants::euler<mp_type>() * i));
    ...
    std::for_each(hypergeometric_2f1_results.begin(), hypergeometric_2f1_results.end(), [](const mp_type& h) {
        std::cout << std::setprecision(DIGIT_COUNT) << h << std::endl;
    });
