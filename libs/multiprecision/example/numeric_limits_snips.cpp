...
BOOST_AUTO_TEST_CASE(test_numeric_limits_snips) {
    ...
    try {
        ...
        */
...

        {
            ...
            s >> read;
            if (read != write) {
                std::cout << std::setprecision(std::numeric_limits<double>::digits10) << read << " != " << write
                          << std::endl;
            }
...

        {
            ...
            for (int i = 2; i < 15; i++) {
                std::cout << std::setw(std::numeric_limits<int>::max_digits10) << boost::math::factorial<double>(i)
                          << std::endl;
            }
    ...
    {
        //[digits10_1
        std::cout << std::setw(std::numeric_limits<short>::digits10 + 1 + 1)    // digits10+1, and +1 for sign.
                  << std::showpos << (std::numeric_limits<short>::max)()        // +32767
                  << std::endl
                  << std::setw(std::numeric_limits<short>::digits10 + 1 + 1) << (std::numeric_limits<short>::min)()
                  << std::endl;    // -32767
                                   //] [/digits10_1]
    }
    ...
    {
        //[digits10_2
        std::cout << std::setw(std::numeric_limits<unsigned short>::digits10 + 1 + 1)    // digits10+1, and +1 for sign.
...

    {
        ...
        */
        ...
        ``
        Similarly we can do the same with NaN (except that we cannot use `assert` (because any comparisons with NaN
        always return false).
        */
        {
            std::locale old_locale;
