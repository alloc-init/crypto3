...
//
//  Contains eval_multiply for cpp_int_modular_backend, which does nothing but converts it to cpp_int_backend and does
//  the multiplication.
//
...
namespace boost {
    namespace multiprecision {
        namespace backends {
            // Functions in this file should be called only for creation of montgomery and Barett params, calculation of
            // inverse element and montgomery_reduce. Since these functions are relatively slow and are not called very
            // often, we will not optimize them. We do NOT care about the execution speed, and will just redirect calls
            // to normal boost::cpp_int.
            template<unsigned Bits1, unsigned Bits2>
            inline BOOST_MP_CXX14_CONSTEXPR void eval_multiply(cpp_int_modular_backend<Bits1 + Bits2> &result,
            ...
            inline BOOST_MP_CXX14_CONSTEXPR
                typename std::enable_if<!boost::multiprecision::backends::is_trivial_cpp_int_modular<
                    cpp_int_modular_backend<Bits1>>::value>::type
            ...
            // We need to specialize this for goldilock fields, where the second argument is trivial, while the first
            // may not. Caller is responsible for the result to fit in "Bits1" bits, we WILL NOT THROW!!!
            template<unsigned Bits1, unsigned Bits2>
            inline BOOST_MP_CXX14_CONSTEXPR typename std::enable_if<
                ...
                boost::multiprecision::backends::eval_multiply(result_cpp_int,
                                                               boost::multiprecision::limb_type(*o.limbs()));
                result.from_cpp_int(result_cpp_int);
            }
