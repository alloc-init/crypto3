...
namespace boost {
    namespace multiprecision {
        ...
        inline BOOST_MP_CXX14_CONSTEXPR typename std::enable_if<
            ...
            number<boost::multiprecision::backends::modular_adaptor<Backend, modular_params_type>>>::type
            powm(const number<boost::multiprecision::backends::modular_adaptor<Backend, modular_params_type>>& b,
                 const U& p) {

            // We will directly call eval_powm here, that's what a call through a default_ops::powm_func would do if
            // expression tempaltes are off. We don't want to change that structure.
