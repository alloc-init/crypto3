...
namespace boost {
    namespace multiprecision {
        namespace backends {
            ...
            BOOST_MP_CXX14_CONSTEXPR void eval_convert_to(Backend *result,
            ...
            BOOST_MP_CXX14_CONSTEXPR void
                eval_subtract(modular_adaptor<cpp_int_modular_backend<Bits>, ModularParamsType> &result,
            ...
            BOOST_MP_CXX14_CONSTEXPR bool
                eval_is_zero(const modular_adaptor<Backend, ModularParamsType> &val) BOOST_NOEXCEPT {
            ...
            BOOST_MP_CXX14_CONSTEXPR void assign_components(modular_adaptor<Backend, ModularParamsType> &result,
                                                            const T &a, const V &b) {
                result.base_data() = a;
                result.mod_data() = b;
                result.mod_data().adjust_modular(result.base_data());
            ...
            inline BOOST_MP_CXX14_CONSTEXPR void eval_left_shift(modular_adaptor<Backend, ModularParamsType> &t,
                                                                 UI i) noexcept {
                Backend tmp;
            ...
            template<typename Backend, typename ModularParamsType>
            BOOST_MP_CXX14_CONSTEXPR bool eval_bit_test(const modular_adaptor<Backend, ModularParamsType> &m,
                                                        std::size_t index) {
                Backend tmp;
                m.mod_data().adjust_regular(tmp, m.base_data());
            ...
            template<typename Backend, typename ModularParamsType>
            BOOST_MP_CXX14_CONSTEXPR void eval_bit_set(modular_adaptor<Backend, ModularParamsType> &result,
                                                       std::size_t index) {
                Backend tmp;
            ...
            // We must make sure any call with any integral type ends up here, if we use std::size_t here, something
            // this function is not preferred by the compiler and boost's version is used, which is worse.
            template<typename Backend, typename ModularParamsType>
            BOOST_MP_CXX14_CONSTEXPR void eval_bit_unset(modular_adaptor<Backend, ModularParamsType> &result,
                                                         std::size_t index) {
                Backend tmp;
                result.mod_data().adjust_regular(tmp, result.base_data());
            ...
            template<typename Backend, typename ModularParamsType>
            BOOST_MP_CXX14_CONSTEXPR void eval_bit_flip(modular_adaptor<Backend, ModularParamsType> &result,
                                                        std::size_t index) {
                Backend tmp;
                result.mod_data().adjust_regular(tmp, result.base_data());
            ...
            inline BOOST_MP_CXX14_CONSTEXPR std::size_t
                hash_value(const modular_adaptor<Backend, ModularParamsType> &val) noexcept {
        ...
        template<class Backend, typename ModularParamsType>
        BOOST_MP_CXX14_CONSTEXPR void
            generic_interconvert(Backend &to, const backends::modular_adaptor<Backend, ModularParamsType> &from,
        ...
        template<class Backend, typename ModularParamsType>
        BOOST_MP_CXX14_CONSTEXPR void
            generic_interconvert(number<Backend> &to,
                                 const number<backends::modular_adaptor<Backend, ModularParamsType>> &from,
                                 const std::integral_constant<int, number_kind_integer> & /*to_type*/,
                                 const std::integral_constant<int, number_kind_integer> & /*from_type*/) {
