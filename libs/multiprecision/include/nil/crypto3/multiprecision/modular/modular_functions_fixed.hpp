...
#if defined(CRYPTO3_HAS_BMI2) && defined(CRYPTO3_HAS_ADX) && defined(__x86_64__)
    ...
    namespace multiprecision {
        namespace backends {
            ...
            BOOST_MP_CXX14_CONSTEXPR
                typename std::conditional<is_trivial_cpp_int_modular<Backend>::value,
            ...
            BOOST_MP_CXX14_CONSTEXPR
                typename boost::enable_if_c<!is_trivial_cpp_int_modular<Backend>::value, limb_type>::type
            ...
            template<typename internal_limb_type, typename Backend>
            BOOST_MP_CXX14_CONSTEXPR
                typename boost::enable_if_c<is_trivial_cpp_int_modular<Backend>::value &&
                                            sizeof(typename trivial_limb_type<max_precision<Backend>::value>::type) >=
            ...
            public:
                ...
                BOOST_MP_CXX14_CONSTEXPR typename std::enable_if<std::is_integral<Backend2>::value &&
                                                                 std::is_unsigned<Backend2>::value>::type
                    barrett_reduce(Backend1 &result, Backend2 input) const {
                    using input_backend_type =
                        typename std::conditional<bool(sizeof(Backend2) * CHAR_BIT > Bits),
                                                  cpp_int_modular_backend<sizeof(Backend2) * CHAR_BIT>, Backend>::type;
                ...
                template<typename Backend1, typename Backend2,
                         typename boost::enable_if_c<
                             ...
                             BOOST_MP_CXX14_CONSTEXPR void barrett_reduce(Backend1 &result, const Backend2 &input)
                                 const {
                ...
                BOOST_MP_CXX14_CONSTEXPR void montgomery_mul(Backend &result, const Backend &y,
                    ...
                    if (m_no_carry_montgomery_mul_allowed)
                        ...
                        montgomery_mul_CIOS_impl(result, y, std::integral_constant<bool, false>());
                }
...
#if defined(CRYPTO3_HAS_BMI2) && defined(CRYPTO3_HAS_ADX) && defined(__x86_64__)
                    if constexpr (operand_limbs == 4 && std::numeric_limits<internal_limb_type>::digits == 64) {
                        // Reuse fp12_fast's ADX/BMI2 raw multiplier, but retain the extra accumulator limb: the sum
                        // may exceed the eight limbs accepted by the assembly Montgomery-reduction kernel.
                        internal_limb_type *accumulator_limbs = accumulator.limbs();
                        for (; first1 != last1; ++first1, ++first2) {
                            BOOST_ASSERT(eval_lt(*first1, m_mod) && eval_lt(*first2, m_mod));
                            std::array<internal_limb_type, 2 * operand_limbs> product;
                            nil::crypto3::multiprecision::detail::schoolbook_4x4_adx_bmi2(
                                product.data(), (*first1).limbs(), (*first2).limbs());

                            internal_limb_type carry = 0;
                            for (std::size_t i = 0; i < product.size(); ++i) {
                                const internal_double_limb_type partial =
                                    static_cast<internal_double_limb_type>(accumulator_limbs[i]) + product[i] + carry;
                                accumulator_limbs[i] = static_cast<internal_limb_type>(partial);
                                carry = static_cast<internal_limb_type>(partial >> limb_bits);
                            }
                            const internal_double_limb_type top =
                                static_cast<internal_double_limb_type>(accumulator_limbs[2 * operand_limbs]) + carry;
                            accumulator_limbs[2 * operand_limbs] = static_cast<internal_limb_type>(top);
                            BOOST_ASSERT((top >> limb_bits) == 0);
                        }
                    } else
#endif
                    {
                        ...
                        while (first1 != last1) {
                            ...
                            for (; first1 != last1 && block_terms < max_block_terms;
                                 ++first1, ++first2, ++block_terms) {
                                BOOST_ASSERT(eval_lt(*first1, m_mod) && eval_lt(*first2, m_mod));
                                ...
                                // propagated only after the block instead of forming a dependency chain in every
                                // product.
                            ...
                            for (std::size_t diagonal = 0; diagonal < product_diagonals; ++diagonal) {
                                ...
                                partial = static_cast<internal_double_limb_type>(diagonal_middle[diagonal]) +
                                          carry_high + low_overflow;
                                carry_low = static_cast<internal_limb_type>(partial);
                                const internal_limb_type middle_overflow =
                                    static_cast<internal_limb_type>(partial >> limb_bits);

                                partial =
                                    static_cast<internal_double_limb_type>(diagonal_high[diagonal]) + middle_overflow;
                ...
                static BOOST_MP_CXX14_CONSTEXPR void dbl_limb_to_limbs(const internal_double_limb_type &X,
                ...
                BOOST_MP_CXX14_CONSTEXPR
                    typename boost::enable_if_c<!is_trivial_cpp_int_modular<Backend1>::value, bool>::type
                    ...
                    // bit in the number. I.E. if modulus is 255 bits, then we have 1 additional "unused" bit in the
                    // number.
                ...
                template<typename Backend1>
                BOOST_MP_CXX14_CONSTEXPR void montgomery_mul_no_carry_impl(Backend1 &c, const Backend1 &b) const {
...
#define MONTGOMERY_MUL_NO_CARRY_LOOP_BODY(X)                             \
    ...
    modular_functions_fixed::dbl_limb_to_limbs(tmp, C, result_limbs[X - 1]);
                ...
                template<typename Backend1>
                BOOST_MP_CXX14_CONSTEXPR void
                    montgomery_mul_CIOS_impl(Backend1 &result, const Backend1 &y,
                                             std::integral_constant<bool, false> const &) const {
...
#define MONTGOMERY_MUL_CIOS_LOOP_BODY(X)                                                                      \
    t = static_cast<internal_double_limb_type>(y_limbs[X]) * static_cast<internal_double_limb_type>(x_i) +    \
        A_limbs[X] + k;                                                                                       \
    t2 = static_cast<internal_double_limb_type>(mod_limbs[X]) * static_cast<internal_double_limb_type>(u_i) + \
    ...
    k2 = static_cast<internal_limb_type>(t2 >> std::numeric_limits<internal_limb_type>::digits);
                        ...
                        internal_double_limb_type tmp = static_cast<internal_double_limb_type>(carry) + k + k2;
                        A_limbs[mod_size - 1] = static_cast<internal_limb_type>(tmp);
                        carry = static_cast<internal_limb_type>(tmp >> std::numeric_limits<internal_limb_type>::digits);
                    }
                ...
                template<typename Backend1>
                BOOST_MP_CXX14_CONSTEXPR void
                    montgomery_mul_CIOS_impl(Backend1 &result,
                                             const Backend1 &y,
                                             std::integral_constant<bool, true> const &) const {
                    ...
                    for (size_t i = 0; i < mod_size; i++) {
                        ...
                        internal_double_limb_type tmp = static_cast<internal_double_limb_type>(
                        ...
                        custom_set_limb_value<internal_limb_type>(
                            A, mod_size,
                            static_cast<internal_limb_type>(tmp >> std::numeric_limits<internal_limb_type>::digits));
                    }
