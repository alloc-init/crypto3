...
namespace detail {
    ...
    typename boost::multiprecision::detail::expression<tag, Arg1, Arg2, Arg3, Arg4>::result_type
        ...
        typedef
            typename boost::multiprecision::detail::expression<tag, Arg1, Arg2, Arg3, Arg4>::result_type result_type;
...
template<class Real, class Val>
typename std::enable_if<boost::multiprecision::number_category<Real>::value !=
                        boost::multiprecision::number_kind_complex>::type
    ...
    if (std::numeric_limits<Real>::digits > std::numeric_limits<Int>::digits) {
        ...
        if (is_checked_cpp_int<Real>::value) {
            BOOST_CHECK_THROW(m.template convert_to<Int>(), std::overflow_error);
                   ...
                   boost::multiprecision::detail::is_integral<Int>::value) {
            r = m.template convert_to<Int>();
            BOOST_CHECK_EQUAL(r, (std::numeric_limits<Int>::max)());
        } else {
            r = m.template convert_to<Int>();
...
struct lexical_cast_target_type {
    typedef typename std::conditional<
        ...
        std::intmax_t,
        typename std::conditional<boost::multiprecision::detail::is_unsigned<T>::value, std::uintmax_t, T>::type>::type
        type;
};
...
void test_negative_mixed_numeric_limits(std::integral_constant<bool, true> const&) {
    ...
    std::ios_base::fmtflags f =
        std::is_floating_point<Num>::value ? std::ios_base::scientific : std::ios_base::fmtflags(0);
...
void test_negative_mixed(std::integral_constant<bool, true> const&) {
    typedef typename std::conditional<
        ...
        typename std::conditional<boost::multiprecision::detail::is_integral<Num>::value && (sizeof(Num) < sizeof(int)),
                                  int, Num>::type,
        Real>::type cast_type;
    typedef typename std::conditional<std::is_convertible<Num, Real>::value, Num, Real>::type simple_cast_type;
    ...
    test_negative_mixed_numeric_limits<Real, Num>(
        std::integral_constant<bool, std::numeric_limits<Real>::is_specialized>());
    // Assignment:
    Real r(0);
...
void test_mixed_numeric_limits(const std::integral_constant<bool, true>&) {
    ...
    if (std::numeric_limits<Real>::has_infinity && std::numeric_limits<Num>::has_infinity) {
        ...
        d = static_cast<Real>(negate_value(std::numeric_limits<Num>::infinity(),
                                           std::integral_constant<bool, std::numeric_limits<Num>::is_signed>()));
        BOOST_CHECK_LT(d, negate_value((std::numeric_limits<Real>::max)(),
                                       std::integral_constant<bool, std::numeric_limits<Real>::is_signed>()));
    }
...
void test_mixed(const std::integral_constant<bool, true>&) {
    ...
    typedef std::integral_constant<
        bool, (!std::numeric_limits<Num>::is_specialized || std::numeric_limits<Num>::is_signed) &&
...
template<class T>
typename std::enable_if<boost::multiprecision::number_category<T>::value ==
                        boost::multiprecision::number_kind_complex>::type
...
void test() {
    ...
    std::integral_constant<bool,
                           boost::multiprecision::is_number<Real>::value && !std::is_same<related_type, Real>::value>
        tag2;

    test_mixed<Real, related_type>(tag2);

    std::integral_constant<bool, boost::multiprecision::is_number<Real>::value &&
                                     (boost::multiprecision::number_category<Real>::value ==
